...
module.exports = function(grunt) {
  ...
  grunt
      .initConfig({
        ...
        buildcontrol : {
          ...
          testing : {
            options : {
              remote : 'git@heroku.com:DemoApp-qa.git',
              branch : 'master'
            }
          },
          develop : {
            options : {
              remote : 'git@heroku.com:DemoApp-dev.git',
              branch : 'master'
            }
          },
          stage : {
            options : {
              remote : 'git@heroku.com:DemoApp-stage.git',
              branch : 'master'
            }
          }
        /*
         * , openshift: { options: { remote: 'openshift', branch:
         * 'master' } }
         */
        },
        ...
        lesslint:{
          src: ['<%= yeoman.client %>/{app,components}/**/*.less']
        }
      });
  ...
  grunt.registerTask('test', function(target) {
    if (target === 'server') {
      ...
      return grunt.task.run([ 'clean:server', 'env:all', 'injector:less',
          'concurrent:test', 'injector', 'autoprefixer', 'karma' ]);
    }
  ...
  grunt.registerTask('deploy-prod', ['clean', 'build', 'buildcontrol:prod']);
  grunt.registerTask('deploy-dev', ['clean', 'build', 'buildcontrol:develop']);
  grunt.registerTask('deploy-qa', ['clean', 'build', 'buildcontrol:testing']);
  grunt.registerTask('deploy-stage', ['clean', 'build', 'buildcontrol:stage']);
